...
import model.DentistaHairoBL;
import model.EquipoDentalHairoBL;
...
public class DlgEquipoDental extends JDialog implements ActionListener {
	...
	void cargarDentistas() {
		...
		List<DentistaHairoBL> lista = em.createQuery("SELECT d FROM DentistaHairoBL d", DentistaHairoBL.class).getResultList();
		cboDentistas.removeAllItems();
		for (DentistaHairoBL d : lista) {
	...
	void listar() {
		...
		List<EquipoDentalHairoBL> lista = em.createQuery(
			"SELECT e FROM EquipoDentalHairoBL e ORDER BY e.nroEquipo DESC", 
			EquipoDentalHairoBL.class
		).getResultList();

		for (EquipoDentalHairoBL e : lista) {
			txtSalida.append("Nro. Equipo.....: " + e.getNroEquipo() + "\n");
			txtSalida.append("Nombre..........: " + e.getNombre() + "\n");
			txtSalida.append("Costo...........: S/. " + e.getCosto() + "\n");
			txtSalida.append("Fecha ..........: " + e.getFechaAdquisicion() + "\n");
			txtSalida.append("Estado..........: " + e.getEstado() + "\n");
			txtSalida.append("Dentista........: " + e.getDentista().getId() + " - " + e.getDentista().getNombreCompleto() + "\n");
			txtSalida.append("-------------------------------------------------------\n\n");
		}
	...
	void adicionar() {
		...
		String fecha = txtFechaAdquisicion.getText();
		String estado = (String) cboEstados.getSelectedItem();
		DentistaHairoBL dentista = (DentistaHairoBL) cboDentistas.getSelectedItem();

		EquipoDentalHairoBL equipo = new EquipoDentalHairoBL();
		equipo.setNombre(nombre);
		equipo.setCosto(costo);
		try {
			equipo.setFechaAdquisicion(new SimpleDateFormat("yyyy-MM-dd").parse(fecha));
		} catch (Exception e) {
			return;
		}
	...
	void buscar() {
		try {
			...
			EntityManager em = JPAUtil.getEntityManager();
			EquipoDentalHairoBL e = em.find(EquipoDentalHairoBL.class, nro);
			if (e == null) {
				mensajeError("Equipo Dental no encontrado");
			} else {
				txtNombre.setText(e.getNombre());
				txtCosto.setText(String.valueOf(e.getCosto()));
				txtFechaAdquisicion.setText(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(e.getFechaAdquisicion()));
			...
			mensajeError("Error al buscar equipo dental: " + e.getMessage());
		}
	}
	...
	void modificar() {
		try {
			...
			e.setFechaAdquisicion(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(txtFechaAdquisicion.getText()));
			e.setEstado((String) cboEstados.getSelectedItem());
			e.setDentista((DentistaHairoBL) cboDentistas.getSelectedItem());
			...
			mensajeError("Error al modificar equipo dental: " + e.getMessage());
		}
	}
	...
	void eliminar() {
		try {
			...
			mensajeError("Error al eliminar equipo dental: " + e.getMessage());
		}
	}
	...
	void habilitarEntradas(boolean sino) {
		...
		txtFechaAdquisicion.setEditable(sino); 
